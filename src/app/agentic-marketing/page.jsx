...
const manifestoSections = [
    ...
    {
        content: "To achieve this level of performance, brands must implement the x402 protocol to facilitate instant machine-to-machine payment. The agent reads inventory levels and pricing data through high-performance REST APIs, then settles through x402: the server responds with HTTP status 402 and payment instructions, and the agent retries with a signed payment payload that a facilitator verifies and settles on-chain. This is the bedrock of headless architecture. By separating the presentation layer from the transactional logic, we allow the agent to operate with surgical precision. It does not need to see a website to execute a purchase. It only needs the raw data."
    },
    {
